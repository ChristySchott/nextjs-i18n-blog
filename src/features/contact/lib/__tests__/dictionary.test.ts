import { getContactDictionary } from '../dictionary'
import type { Locale } from '@/shared/lib/i18n'

jest.mock('@/features/contact/dictionaries/en-US.json', () => ({
  metadata: {
    title: 'My Blog',
  },
}))

jest.mock('@/features/contact/dictionaries/pt-BR.json', () => ({
  metadata: {
    title: 'Meu Blog',
  },
}))

jest.mock('@/features/contact/dictionaries/ar.json', () => ({
  metadata: {
    title: 'مدونتي',
  },
}))

describe('getContactDictionary', () => {
  describe('valid locales', () => {
    it('should return English dictionary for en-US locale', async () => {
      const dictionary = await getContactDictionary('en-US')
    ...
    it('should return Portuguese dictionary for pt-BR locale', async () => {
      const dictionary = await getContactDictionary('pt-BR')
    ...
    it('should return Arabic dictionary for ar locale', async () => {
      const dictionary = await getContactDictionary('ar')
  ...
  describe('locale-specific content', () => {
    it('should return different content for each locale', async () => {
      const enDict = await getContactDictionary('en-US')
      const ptDict = await getContactDictionary('pt-BR')
      const arDict = await getContactDictionary('ar')
  ...
  describe('default locale', () => {
    it('should use default en-US locale if none is provide', async () => {
      const enDict = await getContactDictionary('en-US')
      const dictionary = await getContactDictionary(
  ...
  describe('caching behavior', () => {
    it('should return same structure on repeated calls', async () => {
      const dict1 = await getContactDictionary('en-US')
      const dict2 = await getContactDictionary('en-US')
